...
public class HumlaService extends Service implements IHumlaService, IHumlaSession, HumlaConnection.HumlaConnectionListener, HumlaLogger, BluetoothScoReceiver.Listener {
    ...
    /** Voice hold time in milliseconds for voice activity detection mode. */
    public static final String EXTRAS_VOICE_HOLD_TIME = "voice_hold_time";
...
//            auth.addCeltVersions(Constants.CELT_11_VERSION);
    ...
    public boolean configureExtras(Bundle extras) throws AudioException {
        ...
        if (extras.containsKey(EXTRAS_VOICE_HOLD_TIME)) {
            mActivityInputMode.setVoiceHoldTime(extras.getInt(EXTRAS_VOICE_HOLD_TIME));
        }
    ...
    public boolean injectAudioData(short[] audioData) {
        if (mAudioHandler != null && mAudioHandler.isInitialized()) {
            return mAudioHandler.injectAudioData(audioData);
        }
        Log.w(TAG, "Cannot inject audio: AudioHandler not initialized");
        return false;
    }
    ...
    /**
     * Update the voice hold time for voice activity detection.
     * @param milliseconds Time in milliseconds to continue transmitting after voice stops.
     */
    public void setVoiceHoldTime(int milliseconds) {
        if (mActivityInputMode != null) {
            mActivityInputMode.setVoiceHoldTime(milliseconds);
        }
    }
    
    /**
     * Update the VAD threshold for voice activity detection.
     * @param threshold Detection threshold (0.0 - 1.0).
     */
    public void setVADThreshold(float threshold) {
        if (mActivityInputMode != null) {
            mActivityInputMode.setThreshold(threshold);
        }
    }
    
    /**
     * Change the transmission mode dynamically.
     * @param mode One of Constants.TRANSMIT_PUSH_TO_TALK, TRANSMIT_VOICE_ACTIVITY, or TRANSMIT_CONTINUOUS.
     */
    public void setTransmitMode(int mode) {
        mTransmitMode = mode;
        IInputMode inputMode;
        switch (mode) {
            case Constants.TRANSMIT_PUSH_TO_TALK:
                inputMode = mToggleInputMode;
                Log.i(TAG, "setTransmitMode: Setting PTT mode");
                break;
            case Constants.TRANSMIT_CONTINUOUS:
                inputMode = mContinuousInputMode;
                Log.i(TAG, "setTransmitMode: Setting continuous mode");
                break;
            case Constants.TRANSMIT_VOICE_ACTIVITY:
                inputMode = mActivityInputMode;
                Log.i(TAG, "setTransmitMode: Setting VAD mode");
                break;
            default:
                throw new IllegalArgumentException("Invalid transmit mode: " + mode);
        }
        
        // Set the input mode on the builder for next initialization
        if (mAudioBuilder != null) {
            mAudioBuilder.setInputMode(inputMode);
        }
        
        // If AudioHandler is already running, we need to reinitialize it with the new mode
        if (mAudioHandler != null && mAudioHandler.isInitialized()) {
            Log.i(TAG, "setTransmitMode: Reinitializing AudioHandler with new mode");
            try {
                // Shutdown current audio handler
                mConnection.removeTCPMessageHandler(mAudioHandler);
                mConnection.removeUDPMessageHandler(mAudioHandler);
                mAudioHandler.shutdown();
                
                // Create new audio handler with new input mode
                mAudioHandler = mAudioBuilder.initialize(
                        mModelHandler.getUser(mConnection.getSession()),
                        mConnection.getMaxBandwidth(), mConnection.getCodec(),
                        mVoiceTargetId);
                ...
                Log.i(TAG, "setTransmitMode: AudioHandler reinitialized successfully");
            } catch (AudioException e) {
                Log.e(TAG, "setTransmitMode: Failed to reinitialize AudioHandler", e);
            } catch (NotSynchronizedException e) {
                Log.e(TAG, "setTransmitMode: Not synchronized when reinitializing", e);
            }
        }
    }
    ...
    public void setInputGain(float gain) {
        // Try to set gain directly on AudioHandler, even if not fully synchronized
        if (mAudioHandler != null) {
            mAudioHandler.setInputGain(gain);
            Log.i("HumlaService", "Set input gain to: " + gain + " (direct access)");
        } else {
            Log.w("HumlaService", "Cannot set input gain: AudioHandler not initialized yet");
        }
    }
    
    @Override
    public void setMicBoost(boolean enabled) {
        // Set mic boost directly on AudioHandler
        if (mAudioHandler != null) {
            mAudioHandler.setMicBoost(enabled);
            Log.i("HumlaService", "Mic boost " + (enabled ? "ENABLED" : "DISABLED") + " (direct access)");
        } else {
            Log.w("HumlaService", "Cannot set mic boost: AudioHandler not initialized yet");
        }
    }
    ...
    public void setOutputGain(float gain) {
        // Try to set gain directly on AudioHandler, even if not fully synchronized
        if (mAudioHandler != null) {
            mAudioHandler.setOutputGain(gain);
            Log.i("HumlaService", "Set output gain to: " + gain + " (direct access)");
        } else {
