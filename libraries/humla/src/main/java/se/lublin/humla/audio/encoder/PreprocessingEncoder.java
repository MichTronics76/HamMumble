...
public class PreprocessingEncoder implements IEncoder {
    ...
    private boolean mBypass = false; // Flag to bypass preprocessing
    ...
    public int encode(short[] input, int inputSize) throws NativeAudioException {
        // Only preprocess if bypass is not enabled
        if (!mBypass) {
    ...
    /**
     * Enable or disable preprocessing bypass
     * Use this to send clean audio (e.g., tones, roger beeps) without filtering
     */
